...
pub enum ContractError {
    ...
    RecurringPaymentNotFound = 39,
    InvalidInterval = 40,
    ScheduleNotActive = 41,
    DisbursementTooEarly = 42,
    ScheduleEnded = 43,
    TooManyActiveRecurring = 44,
    ScheduleAlreadyCancelled = 45,
}
...
impl AccordContract {
    ...
    pub fn disburse_recurring(env: Env, schedule_id: u64) -> Result<(), ContractError> {
        require_not_frozen(&env)?;
        ...
        if schedule.end_time > 0 && now >= schedule.end_time {
    ...
    /// Maintenance function to sweep completed recurring schedules and decrement
    /// the active recurring schedule count (`ACTREC`), freeing slots for new schedules.
    pub fn sweep_completed_recurring(
        env: Env,
        caller: Address,
        ids: Vec<u64>,
    ) -> Result<u32, ContractError> {
        ...
        for id in ids.iter() {
            let schedule = match read_recurring_payment(&env, id) {
                Ok(s) => s,
                Err(_) => continue,
            };

            if matches!(derive_recurring_status(&env, &schedule), RecurringStatus::Completed) {
                swept = swept.saturating_add(1);
            }
        }
        ...
        if swept > 0 {
            let active = read_active_recurring_count(&env);
            if active > 0 {
                write_active_recurring_count(&env, active.saturating_sub(swept));
            }
        }
    ...
    /// Returns the amount currently claimable for a schedule based on its type and elapsed time.
    pub fn get_claimable_amount(env: Env, schedule_id: u64) -> Result<i128, ContractError> {
        let schedule = read_recurring_payment(&env, schedule_id)?;
